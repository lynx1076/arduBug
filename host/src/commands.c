...
int cmd_mem_read(size_t arg_cnt, const char** tokens) {
  ...
  if (count == 1) {
    ...
    if (dev_mem_page_read(addr, count, data)) return -1;
  }
...
int cmd_mem_write(size_t arg_cnt, const char** tokens) {
  ...
  if (count == 1) {
    ...
    if (dev_mem_page_write(addr, count, data)) return -1;
  }
...
int cmd_test(size_t arg_cnt, const char** tokens) {
  (void)arg_cnt;
  (void)tokens;

  uint8_t code[] = {
    0xa9, 0xff, 0x8d, 0x02, 0x60, 0xa9, 0xe0, 0x8d, 0x03, 0x60, 0xa9, 0x38, 0x8d, 0x00, 0x60, 0xa9, 0x00, 0x8d, 0x01, 0x60, 0xa9, 0x80, 0x8d, 0x01, 0x60, 0xa9, 0x00, 0x8d, 0x01, 0x60,
    0xa9, 0x0e, 0x8d, 0x00, 0x60, 0xa9, 0x00, 0x8d, 0x01, 0x60, 0xa9, 0x80, 0x8d, 0x01, 0x60, 0xa9, 0x00, 0x8d, 0x01, 0x60, 0xa9, 0x06, 0x8d, 0x00, 0x60, 0xa9, 0x00, 0x8d, 0x01, 0x60,
    0xa9, 0x80, 0x8d, 0x01, 0x60, 0xa9, 0x00, 0x8d, 0x01, 0x60, 0xa9, 0x48, 0x8d, 0x00, 0x60, 0xa9, 0x20, 0x8d, 0x01, 0x60, 0xa9, 0xa0, 0x8d, 0x01, 0x60, 0xa9, 0x20, 0x8d, 0x01, 0x60,
    0x4c, 0x5a, 0x80
  };

  const int code_len = sizeof(code) / sizeof(*code);

  printf("Flashing: %i bytes\n", code_len);

  if (devm_bulk_write(0x8000, code_len, code)) return -1;
